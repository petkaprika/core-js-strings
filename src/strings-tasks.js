...
function isPalindrome(str) {
  const symbols = str
    ...
    .split(/[,.:!? ]/)
    .join('');
  return symbols === reverseString(symbols);
}
...
function findLongestWord(sentence) {
  ...
  const maxLength = words.reduce(
    (length, word) => (word.length > length ? word.length : length),
    0
  );
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((value) => {
      const index = strInput.indexOf(value);
      return index === -1 ? value : strOutput.at(index);
    })
    .join('');
}
